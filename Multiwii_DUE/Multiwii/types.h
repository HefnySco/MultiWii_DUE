...
struct __attribute__ ((packed)) pid_  {
  uint8_t P8;
  uint8_t I8;
  uint8_t D8;
};

struct  __attribute__ ((packed)) servo_conf_ {  // this is a generic way to configure a servo, every multi type with a servo should use it
