...
boolean SAM3X8E_EEFC::writeData(void *data, uint32_t len, const void *_flash)
{
	...
	uint32_t endOffset   = ((flash + len - start - 1) % pageSize) + 1;
	...
	for (int page=startPage; page<=endPage; page++)
	{
		...
		if (page==startPage)
		for (int off=0; off<start; off++) {      //off<=start  ===>    off<start
		...
		// this for loop is executed once
		if (page==endPage)
		for (int off=end; off<pageSize/4; off++) { // write after the end... as end 4 bytes are included in data.
